...
#        word =row[0] # the word
#        pos = row[1] # the tag
file = open('NewsQur.txt', mode="r") 
for line in file: 
    print("line= ", line)
    wint64 = word2int64(line)  # RIT64 encoding for words
    wbin = bin(wint64)         # binary format 
#    pint64= word2int64(pos)    #RIT64 for POS tags
#    pbin = bin(pint64)         #binary format 
   
    wordList.append(line)      # create the word list
...
def train (wordList,posList, thing, XAA, tag):
...
#    pts_onehot_encoded = pts_onehot_encoder.fit_transform(pts_integer_encoded)
    ...
    model.add(Dense(thing, activation='sigmoid'))
    XAtr, XAts, Ytr, Yts = train_test_split( XAA, y1, test_size=0.25, random_state=87)
    model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
    model.fit(XAtr, Ytr, epochs=40, batch_size=10)
    scores = model.evaluate(XAts, Yts)
    global results
    results+= "%s, %s: %.2f%%\n" % (tag, model.metrics_names[1], scores[1]*100)


#first half
train(wordList1, 83, XAA1, "First half of the corpus")

#second half
train(wordList2, 73, XAA2, "Second half of the corpus")

#full corpus
train(wordList,88, XAA, "Full corpus")
print(results)